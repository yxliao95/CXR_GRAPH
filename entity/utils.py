...
def batchify(samples, batch_size):
    ...
    # if a sentence (document?) is too long, make itself a batch to avoid GPU OOM
    to_single_batch = []
    for i, sample in enumerate(samples):
        if len(sample["tokens"]) > 350:
    ...
    for i in to_single_batch:
        ...
        list_samples_batches.append([samples[i]])  # having only one sample in this batch
...
def convert_dataset_to_samples(dataset, max_span_length, ner_label2id=None, context_window=0, split=0):
    ...
    for c, doc in enumerate(dataset):
        ...
        for i, sent in enumerate(doc):
            ...
            if context_window != 0 and len(sent.text) > context_window:
                logger.info("Long sentence: %s %d", sample, len(sent.text))
    ...
    avg_length = sum([len(sample["tokens"]) for sample in samples]) / len(samples)  # including context
